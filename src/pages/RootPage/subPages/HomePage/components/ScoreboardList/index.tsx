...
export default function ScoreboardList() {
    ...
    if (isPending) {
        return Array.from({ length: 10 }, (_, i) => {
            return <Skeleton key={i} variant="rounded" height={105} sx={{ borderRadius: 4 }} />;
        });
    }
    ...
    return (
        <Stack spacing={2}>
            {data
                .sort((scoreboard1, scoreboard2) => scoreboard2.createdAt.valueOf() - scoreboard1.createdAt.valueOf())
                .map((scoreboard) => {
                    return (
                        <ScoreboardItem
