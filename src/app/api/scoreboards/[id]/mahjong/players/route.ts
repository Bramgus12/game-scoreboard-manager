...
import { getMahjongPlayers } from "@/server/service/mahjong";
...
export async function GET(_request: Request, props: Props) {
    try {
        ...
        const players = await getMahjongPlayers(scoreboardId);

        return NextResponse.json(players);
    } catch (error) {
        logApiError({
            route: "/api/scoreboards/[id]/mahjong/players",
        ...
        const message =
            error instanceof Error
                ? error.message
                : "Failed to fetch mahjong players";
